...
### 회고
처음에는 테두리 없이 전체 탐색을 진행하는 방향으로 갔다가 코드가 너무 복잡해져서 테두리를 만드는 방법을 떠올림. 그리고 나서 시간이 너무 오래걸리는 문제가 있어서 어떻게 수정할지 찾아보다가 너비 우선 탐색을 알게되었고 이를 적용해본 문제

### 성능 요약

메모리: 128016 KB, 시간: 568 ms
