...
아스키코드로 변환하여 순서를 매겨 원하는 답을 구하는 문제.
이 때 'a'는 1부터 시작하기 때문에 본 풀이에서는 1을 보정해주는 과정에서 에러가 많이 발생하였고,
첫 변환 리스트를 sorted를 해주어 ban된 글자만큼 뒷순서로 옮겨가는 과정에서의 에러를 해결함

### 성능 요약

메모리: 173 MB, 시간: 373.19 ms
